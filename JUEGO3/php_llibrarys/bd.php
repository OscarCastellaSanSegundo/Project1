...
{
    $conexion = openBd();
    $sentenciaText = "select * from usuario_juego_curso INNER JOIN usuario ON usuario.Id = usuario_juego_curso.IdUsuario  where idJuego=3 ORDER BY Puntuacion DESC LIMIT 5;";
    $sentencia = $conexion->prepare($sentenciaText);
    $sentencia->execute();
    $resultado = $sentencia->fetchAll();
    $conexion = closeBd();
...
function insertarPuntuacion($puntuacion)
{
    ...
    if ($puntuacion == null) {
        ...
        while ($i < $longitudArray) {
            if ($resultado[$i]['IdUsuario'] == $sessionActual && $resultado[$i]['IdJuego'] == 3 && $resultado[$i]['Puntuacion'] > $puntuacion) {
                $cambiarPuntuacion = 2;
                $i = $longitudArray + 1;
            } else if ($resultado[$i]['IdUsuario'] == $sessionActual && $resultado[$i]['IdJuego'] == 3 && $resultado[$i]['Puntuacion'] <= $puntuacion) {
                $cambiarPuntuacion = 1;
                $i = $longitudArray + 1;
            } else {
                if ($puntuacion == null) {
        ...
        if ($cambiarPuntuacion == 0) {
            ...
            $sentenciaTexto = "insert into usuario_juego_curso values (null, 3, :id_usuario, 3, :puntuacion )";
            $sentencia = $conexion->prepare($sentenciaTexto);
            ...
            $sentenciaTexto = "update usuario_juego_curso set Puntuacion = :puntuacion where IdUsuario = :id_usuario and IdJuego = 3";
            $sentencia = $conexion->prepare($sentenciaTexto);
