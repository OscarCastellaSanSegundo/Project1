...
function insertarPuntuacion($puntuacion){
    ...
    if ($puntuacion == null) {
        ...
        while ($i < $longitudArray) {
            if ($resultado[$i]['IdUsuario'] == $sessionActual && $resultado[$i]['IdJuego'] == 2 && $resultado[$i]['Puntuacion'] > $puntuacion) {
                $cambiarPuntuacion = 2;
                $i = $longitudArray + 1;
                
            } else if ($resultado[$i]['IdUsuario'] == $sessionActual && $resultado[$i]['IdJuego'] == 2 && $resultado[$i]['Puntuacion'] <= $puntuacion) {
                $cambiarPuntuacion = 1;
                $i = $longitudArray + 1;
        ...
        if ($cambiarPuntuacion == 0) {
            ...
            $sentenciaTexto = "insert into usuario_juego_curso values (null, 2, :id_usuario, 3, :puntuacion )";
            $sentencia = $conexion->prepare($sentenciaTexto);
            ...
            $sentenciaTexto = "update usuario_juego_curso set Puntuacion = :puntuacion where IdUsuario = :id_usuario and IdJuego = 2";
            $sentencia = $conexion->prepare($sentenciaTexto);
